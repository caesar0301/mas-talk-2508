...
Migrated from cogent_st.agents.researcher.researcher for reuse.
"""
        ...
        **kwargs,
        ...
        try:
            ...
            if not grounding_chunks:
                raise GoogleAISearchError("No grounding chunks found in the response")

            # Extract citations and add them to the generated text
            citations = self._get_citations(response)
            cited_text = self._insert_citation_markers(response.text, citations)

            logger.info(f"GoogleAISearch completed for query: {query} with {len(citations)} citations")

            # Prepare sources gathered from citations
            sources_gathered = []
            for citation in citations:
                for segment in citation.get("grounding_chunks", []):
                    sources_gathered.append(segment)
